...
import {
  reminderHistoryFromJson,
  reminderHistoryToJson,
...
export function settingsFromConfig(config: SyncConfig): SyncSettings {
...
export class AppDataStore {
  ...
  async loadCachedSync(): Promise<AppSyncResponse | null> {
    return this.readJson(CACHED_SYNC_KEY, (json) =>
      appSyncResponseFromJson(json),
    );
  }
  ...
  async clearCachedSync(): Promise<void> {
    await this.store.deleteItem(CACHED_SYNC_KEY);
  }

  async loadCourseCatalog(): Promise<CourseCatalog> {
    return (
      (await this.readJson(COURSE_CATALOG_KEY, (json) =>
        courseCatalogFromJson(json),
      )) ?? emptyCourseCatalog
    );
  }
